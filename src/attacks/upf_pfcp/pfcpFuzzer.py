from src.pfcpToolkit.pfcpToolkit import PFCPToolkit
...
class PFCPFuzzer:
    ...
    def start_PFCP_SEID_fuzzing(
        ...
        self.logger.info(
            f"Starting PFCP SEID fuzzing on {upf_addr} with max SEID {max_seid}"
        )
        ...
        if self.verbose:
            self.logger.success(
                f"Fuzzing completed, {len(valid_seid_list)} SEIDs discovered"
            )
