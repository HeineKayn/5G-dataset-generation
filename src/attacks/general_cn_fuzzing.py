...
GENERAL_API_SOURCE_FOLDER = "../5GC_APIs"
...
def extract_ref(original_file:str, ref:str):
    ...
    # Read the content
    file_path = f"{GENERAL_API_SOURCE_FOLDER}/{file}"
    ...
    for key in yaml_content.copy().keys():
        ...
        if key == "$ref" :
            ...
            try:
                ...
                # print(f"Can't find {GENERAL_API_SOURCE_FOLDER}/{ref_file}{path}") 
...
def sample_file(nf:str,k:int) -> list:
    """
        Return a list of random files that concern a certain nf
    """
    nf_file_name = "N" + nf.lower()
    files = [f for f in os.listdir(GENERAL_API_SOURCE_FOLDER) if nf_file_name in f]
    k = min(k,len(files))
...
def general_fuzz(nf_list=["NRF","UDM","AMF"],nb_file=1, nb_url=1, nb_method=1, nb_ite=1, only_required=True, display=True):
    
    request_result_list = []
    for nf in nf_list:
        for file in sample_file(nf,nb_file): 
